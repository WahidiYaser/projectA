let view = document.querySelector("#view") as HTMLDivElement;
let game_Over = document.querySelector("#gameOver") as HTMLDivElement;
let reset = document.querySelector("#reset") as HTMLButtonElement;
let score = document.querySelector("#score") as HTMLSpanElement;
let snake: any[] = [];
let boradDivs: NodeListOf<HTMLDivElement>;
let direction: number = 1;
let playStart: number = 960;
let playGame: any;
let exKeyPressed: KeyboardEvent["key"] = "ArrowRight";
...
function changeDirection(e: KeyboardEvent) {
    ...
    if (keyPressed.key == "ArrowUp") { direction = -52; exKeyPressed = "ArrowUp"; }
    if (keyPressed.key == "ArrowDown") { direction = 52; exKeyPressed = "ArrowDown"; }
    if (keyPressed.key == "ArrowLeft") { direction = -1; exKeyPressed = "ArrowLeft"; }
    if (keyPressed.key == "ArrowRight") { direction = 1; exKeyPressed = "ArrowRight"; }
}
function returnNumber(str: string): number {
...
function moveTheSnake() {
    ...
    for (let i = snake.length - 1; i >= 0; i--) {
        if (i == snake.length - 1) {
            ...
            if (direction > 0) { headPosition += direction; }
            if (direction < 0) { headPosition -= Math.abs(direction); }
    ...
    if (playStart) { boradDivs[playStart].classList.remove("snake"), playStart = 0; } //runs once
    boradDivs[removeTheTailExPosition].classList.remove("snake");
    if (appleEat(headPosition)) { boradDivs[removeTheTailExPosition].classList.remove("snake"); }
}
function throwApple() {
...
function appleEat(headPosition: any) {
